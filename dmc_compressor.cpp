...
int main(int argc, char **argv)
{
    ...
    if (decode_flag)
    {
        ...
        for (uint64_t pos = 0; pos < file_size; ++pos) {
            auto symbol = dmc.get_symbol_bits<8>();
            ...
            for (auto symbol : mmap)
            {
                ...
                if (dmc.get_bytes_count() > 0x100000)
                {
                    ...
                    if (s > 0 && part > 0 && part_packed.size() > 0)
                    {
                        ...
                        std::cout << "\r" << (pos >> 20) << " -> " << (compressed_size >> 20) << " MB, speed: " << speed_mbs << " MB/s, ratio: " << (part / part_packed.size()) << ":1, nodes: " << dmc.get_nodes_count() << "        \b\b\b\b\b\b\b\b" << std::flush;
                    }

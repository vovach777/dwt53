...
struct Band {
    ...
    int heightLL;
    Block blocks[3]; 
    Band(int num, int &width, int &height) : nb(num), width(width), height(height)
    {
...
static std::vector<dwt2d::Band> calc_geometry(int width, int height, int levels )
{
...
class Transform {
    ...
    void set_transform(Transforms transform) {
        if (transform == haar) {
            forward_ = &::haar;
            inverse_ = &::ihaar;
        }  else
...
void prepare_transform( int levels, Transforms transform, the_matrix const &data) {
    data_ = data;
...
the_matrix const & forward()
{        
    for (auto &row : data_) 
    {           
        for (int i=0; i<geometry_.size(); i++) {
            const auto size = geometry_[i].width;
            forward_( row.data(), size, LH_storage_.data(), LH_storage_.data() + sizeof_L(size) );
            std::copy( LH_storage_.begin(), LH_storage_.begin() + size, row.begin() );
        }
    }
    for (int x=0; x < data_[0].size(); x++) {
        ...
        for (int i=0; i<geometry_.size(); i++ ) {
            ...
            forward_( col.data(), size, LH_storage_.data(), LH_storage_.data() + sizeof_L(size) );
            std::copy( LH_storage_.begin(), LH_storage_.begin() + size, col.begin() );
        }
        set_col(data_, x, col);
    }
    return data_;
...
{        
    ...
    for (int x=0; x < data_[0].size(); x++) {
        ...
        for (int i=geometry_.size()-1; i>=0; --i) {
            ...
            inverse_( col.data(), size, LH_storage_.data(), LH_storage_.data() + sizeof_L(size) );            
        }
        set_col(data_, x, col);
    }
    ...
    {           
        for (int i=geometry_.size()-1; i>=0; --i) {
            ...
            inverse_( row.data(), size, LH_storage_.data(), LH_storage_.data() + sizeof_L(size) );            
        }
    }
    return data_;
}
...
static the_matrix compress_data(the_matrix const & data, Transforms wavelet, int levels=5, int Q=16, int details_level=2)
{

    Transform dwt;
    dwt.prepare_transform(levels, wavelet, data);
    return std::move( dwt.forward() );
    
}

static the_matrix decompress_data(the_matrix const & data, Transforms wavelet,  int levels=5, int Q=16, int details_level=2)
{    
    Transform dwt;
    dwt.prepare_transform(levels, wavelet, data);
    return std::move( dwt.inverse() );
}
